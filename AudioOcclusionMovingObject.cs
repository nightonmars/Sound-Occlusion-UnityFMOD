...
public class AudioOcclusionMovingObject : MonoBehaviour
{
            ...
            if (Physics.Raycast(transform.position, directionToPlayer, out hit, maxDistance, obstacleLayer))
            {
                ...
                if (valToStartEvent > 0)
                {
                    if (!playEvent)
                    {
                        ...
                        RuntimeManager.AttachInstanceToGameObject(audioSource, gameObject.transform, rb);
                        if(rb == null)
                        {
                            Debug.Log("Please add a rigidbody");
                        }
                        audioSource.start();
                        playEvent = true;
                    }
